...
async function inspect() {
  ...
  const bbItems = await page.evaluate(() => Array.from(document.querySelectorAll('.news-flash-item')).map(el => ({
  ...
  await new Promise(r => setTimeout(r, 8000));
  const gateItems = await page.evaluate(() => Array.from(document.querySelectorAll('a[href*="/announcements/"], a[href*="/article/"]')).map(a => {
