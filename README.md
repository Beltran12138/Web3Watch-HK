...
> Web3/Crypto 行业聚合 + AI 分类摘要 + 企业微信推送 — 为决策者提供高信噪比情报
...
Alpha-Radar 是一个**面向香港 Web3 合规交易所（VATP 申请者）**的行业情报系统，自动抓取 **20+ 数据源**（交易所公告、KOL 推文、媒体快讯、预测市场），通过 **DeepSeek AI** 进行智能分类、摘要提炼和重要性判定，最终通过 **企业微信** 实时推送重要信号，并提供日报/周报自动生成。
...
| 🤖 **AI 分类** | DeepSeek V3 模型，输出 `business_category`（业务分类）、`competitor_category`（竞品维度）、`detail`（一句话摘要） |
| ⚡ **实时推送** | 企业微信机器人 webhook，仅推送经 AI 判定为"重要"的信号（约占总量的 15-25%） |
...
│           ┌──────────────────┼──────────────────┐                   │
│           │                  │                  │                   │
│    [SQLite 本地缓存]   [Supabase 云端同步]   [企业微信推送]          │
│           │                  │                  │                   │
│    ┌──────┴──────    ┌─────┴─────┐    ┌────────────┐             │
│    │  Web 前端    │    │  多端访问  │    │  领导看板   │             │
│    └─────────────┘    └───────────┘    └─────────────┘             │
...
├── ai.js                     # DeepSeek AI 处理（单条分类 + 批量补充 + 报告生成）
├── filter.js                 # 噪声过滤 + 去重逻辑
